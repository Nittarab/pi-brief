...
export function promptFor(previous: Brief, events: Activity[], outline = false): string {
  ...
  const trace = `Also return key trace as an object, not an array: {"task":"","pivot":"","drift":"","steps":[]}. Do not assign who or kind. task is the same job as goal, shortened to 32 characters, and it must contain the same job noun as goal. It is not a new interpretation and not the latest question. If the source has a Locked task, task must stay that unless the user changed the product. Do not open with an abandoned request. pivot is "" unless the user text contains instead, new task, or forget that. A skill tag is not a previous job. Do not set pivot for the first real job. drift is the wrong job if the agent left the locked task, else "". steps is 2 to 4 current decisions or results, each max 32 characters, present tense. A step must not start with Applied, Repeated, Declared, Asked, Edited, Kicked, Ran, Checked, or Rewrote. Bad step: "Applied edits rewriting the trace". Good step: "rail shows decisions, not chat". Bad task: "Kicked off a standup". Good task: "model trace, not the chat". Do not copy the source messages. Do not quote them. Omit tool names, file paths, and skill tags.`;
