...
- `/trace` shows or hides a right-side rail when the terminal is at least 100 columns wide. The rail covers the chat. It does not shrink it. The session tree is the source. The model presents the trace in the same call as `/brief`. The rail does not copy those messages. `●` is a user decision. `◇` is an agent decision. `!` is a pivot or drift. `↩` is a branch switch. Until the model answers, the rail says `reading`.
