...
- `/trace` shows or hides a right-side rail when the terminal is at least 100 columns wide. The rail covers the chat. It does not shrink it. It is a message trace, not a tool trace: `●` is you, `◇` is the assistant, `!` is drift or a new task, `?` is a possible new ask, and `↩` is a branch switch. Tool names are not shown.
